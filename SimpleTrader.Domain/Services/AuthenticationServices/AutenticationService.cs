...
namespace SimpleTrader.Domain.Services.AuthenticationServices
{
    public class AutenticationService : IAuthenticationService
    {
        ...
        public AutenticationService(IAccountService accountService, IPasswordHasher passwordHasher)
        {
        ...
        public async Task<RegistrationResult> Register(string email, string username, string password, string confirmPassword)
        {
            ...
            {
                result = RegistrationResult.EmailAlreadyExsists;
            }

            Account usernameAccount = await _accountService.GetByUsername(username);
            if (usernameAccount != null)
            {
                result = RegistrationResult.UsernameAlreadyExsists;
            }
